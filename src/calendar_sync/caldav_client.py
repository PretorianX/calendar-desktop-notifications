...
class CalDAVClient:
    ...
    def __init__(
        ...
        # caldav is untyped; keep Optional[Any] and narrow via checks.
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            principal = self.client.principal()  # type: ignore[no-untyped-call]
    ...
    def get_events(
        ...
        try:
            ...
            logger.debug(
                f"Date range in UTC: {start_date.astimezone(pytz.UTC)} to {end_date.astimezone(pytz.UTC)}"
            )
            caldav_events = calendar.date_search(
                start=start_date, end=end_date, expand=True  # Expand recurring events
            )
            ...
            for caldav_event in caldav_events:
                ...
                # NOTE: be defensive here: in tests `vobject_instance` is often a MagicMock, and
                # `hasattr(mock, "vevent_list")` is always True.
                vevent_list: List[Any] = []
                raw_vevent_list = getattr(vobj, "vevent_list", None)
                if isinstance(raw_vevent_list, list) and raw_vevent_list:
                    vevent_list = raw_vevent_list
                    logger.debug(
                        f"Found VCALENDAR with {len(vevent_list)} VEVENT components"
                    )
                ...
                for event_data in vevent_list:
                    ...
                    logger.debug(
                        f"Event attributes: {[attr for attr in dir(event_data) if not attr.startswith('_')]}"
                    )

                    # Debug: Log key iCalendar fields for this event
                    logger.debug(
                        f"Event UID: {event_data.uid.value if hasattr(event_data, 'uid') else 'N/A'}"
                    )
                    logger.debug(
                        f"Event Summary: {event_data.summary.value if hasattr(event_data, 'summary') else 'N/A'}"
                    )
                    logger.debug(
                        f"Event DTSTART: {event_data.dtstart.value if hasattr(event_data, 'dtstart') else 'N/A'}"
                    )
                    logger.debug(
                        f"Event DTEND: {event_data.dtend.value if hasattr(event_data, 'dtend') else 'N/A'}"
                    )
                    logger.debug(
                        "Event RECURRENCE-ID: %s",
                        (
                            event_data.recurrence_id.value
                            if hasattr(event_data, "recurrence_id")
                            else "None"
                        ),
                    )
                    logger.debug(
                        f"Event RRULE: {event_data.rrule.value if hasattr(event_data, 'rrule') else 'None'}"
                    )
                    logger.debug(
                        f"Event SEQUENCE: {event_data.sequence.value if hasattr(event_data, 'sequence') else 'N/A'}"
                    )
                    ...
                    if hasattr(event_data, "recurrence_id"):
                        ...
                        logger.debug(
                            "Event %s is a modified instance of a recurring event (RECURRENCE-ID: %s)",
                            uid,
                            event.recurrence_id,
                        )
                    else:
            ...
            # Filter out events that are more than 1 day before the requested window.
            # This removes old instances of recurring events that might appear in the same VCALENDAR,
            # while keeping results stable/deterministic relative to the query window.
            cutoff_time = start_date - datetime.timedelta(hours=24)
            ...
            for event in events:
                if event.start_time > cutoff_time:
                    ...
                    logger.debug(
                        "Filtering out old event instance: %s at %s (more than 24h in the past)",
                        event.summary,
                        event.start_time,
                    )
            ...
            logger.info(
                f"Filtered from {len(events)} to {len(filtered_events)} events after removing old instances"
            )
